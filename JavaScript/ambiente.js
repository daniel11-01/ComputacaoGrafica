...
export function inicializarAmbiente() {
    ...
    estado.renderer = new THREE.WebGLRenderer({ antialias: false });
    estado.renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1));
    ...
    estado.renderer.shadowMap.type = THREE.PCFShadowMap;
...
export function criarTerreno() {
    ...
    // === MAR: Oceano com ondas via vertex shader (GPU) ===
    var materialOceano = new THREE.ShaderMaterial({
        uniforms: {
            uTime:    { value: 0 },
            uColor:   { value: new THREE.Color(0x0e5e8c) },
            uOpacity: { value: 0.88 }
        },
        vertexShader: [
            'uniform float uTime;',
            ...
            '  float fator = min(1.0, dist / 60.0);',
            '  float h = (sin(dist * 0.04 - uTime * 1.2) * 0.5',
            '           + sin(pos.x * 0.06 + uTime * 0.9) * 0.4',
            '           + sin(pos.y * 0.12 + uTime * 1.6) * 0.15)',
            '           * (0.3 + fator * 0.7);',
            '  pos.z += h;',
...
export function atualizarAmbiente(delta, tempo) {
    ...
    // Ondas do oceano via uniform (cálculo na GPU)
    if (estado.oceano) {
        estado.oceano.material.uniforms.uTime.value = tempo;
    }
}
